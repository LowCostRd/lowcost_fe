...
const savedAgentId = sessionStorage.getItem("draft_agent_id");
const savedAgentName = sessionStorage.getItem("draft_agent_name");
const savedAgentSpecialty = sessionStorage.getItem("draft_agent_specialty");
const savedAgentVoiceId = sessionStorage.getItem("draft_agent_voice_id");
const savedAgentImageUrl = sessionStorage.getItem("draft_agent_image_url");
...
export const useAgentStore = create<AgentState>((set,get) => ({
  ...
  updateAgentVoice: async (agentId: string, voiceId: string, imageUrl?: string) => {
    ...
    try {
      await api.patch(`/v1/api/agents/${agentId}/voice`, { voice_id: voiceId, image_url: imageUrl });
      sessionStorage.setItem("draft_agent_voice_id", voiceId);
      if (imageUrl) {
        sessionStorage.setItem("draft_agent_image_url", imageUrl);
      }
      set({ isLoading: false, agentVoiceId: voiceId, agentImageUrl: imageUrl ?? null });
    } catch (error) {
      set({ isLoading: false });
  ...
  setAgent: ({ agentId, agentName, agentSpecialty }) => {
    sessionStorage.setItem("draft_agent_id", agentId);
    sessionStorage.setItem("draft_agent_name", agentName);
    if (agentSpecialty) sessionStorage.setItem("draft_agent_specialty", agentSpecialty);
    set({ agentId, agentName, ...(agentSpecialty ? { agentSpecialty } : {}) });
  },
